    ...
    fs.readFile(imgpath,(err, photo) =>{
      pool.query('select file_name from photo where file_name=?', [file], (err, data) => {
        ...
        { 
         ...
         pool.query(query, [file.split('_')[1],photo], (err, results) => {
           if (err) throw err;
...
router.delete('/delete', (req, res) => {
  const deletedFileName = req.query.filename;
  // 從資料庫中刪除檔案的邏輯
  const deleteQuery = `DELETE FROM photo WHERE file_name = ?`;

  pool.query(deleteQuery, [deletedFileName], (err, results) => {
    if (err) {
      console.error(err);
      res.status(500).json({ success: false, message: 'Failed to delete file from the server' });
    } else {
      res.json({ success: true, message: 'File deleted successfully from the server' });
    }
  });
});
